...
int main(int argc, char *argv[])
{
    ...
    // 파일 내용을 메모리에 로드
    unsigned char *text = malloc(file_size + 1);
    if (!text)
    {
    ...
    unsigned char *test_texts[] = {
        (unsigned char *)"deep learning",
        (unsigned char *)"natural language processing",
        (unsigned char *)"machine intelligence",
        (unsigned char *)"large-scale dataset"};
